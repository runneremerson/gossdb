...
func Start(config ...string) error {
	fmt.Println("SSDB Pool Setting up")
	if len(config) > 0 {
		ConfigName = config[0]
	}
	...
	if err == nil {
		...
		fmt.Printf("SSDB configuring file %s not found, using default configuring values for setting up\n", ConfigName)
	}
...
func Close() {
	...
	pool = nil
	fmt.Println("SSDB Pool is shutdown")
}
...
func Client() (*gossdb.Client, error) {
	if pool == nil {
		return nil, fmt.Errorf("SSDB Pool has not inited")
	}
